document.addEventListener('DOMContentLoaded', () => {
  ...
  function displayGifts(filteredGifts) {
    ...
    filteredGifts.forEach(gift => {
      ...
      card.setAttribute('data-name', gift.name); // Добавляем data-name для идентификации карточки
      card.innerHTML = `
        <div class="img-container">
          <img src="${gift.imgSrc}" alt="${gift.category}" class="card-img" />
        </div>
        <div class="gift-text-container">
          <h4 class="${gift.category}">${capitalizeFirstLetter(
        `For ${gift.category}`
      )}</h4>
          <h3>${gift.name}</h3>
        </div>
